...
- 2026-07-23 将全量重建改为 `.next` 暂存索引、完整提交后切换，并保留 `.previous` 回滚目录；自动化测试验证构建期间旧快照持续可查、切换后只暴露新快照、进程中断后恢复上一完整快照。
- 2026-07-23 在 Windows D 盘 2,808,309 条 MFT 记录、2,526,551 个文件上执行 release 快速阶段验收：MFT 枚举 7.651 秒，首批可搜索 8.318 秒，完整可搜索索引 30.208 秒，查询 2 毫秒。
- 2026-07-23 同机记录索引资源对比：Everything 数据库为 245,093,385 字节、服务进程工作集约 525 MB；LogCrate 当前 C/D 双卷 SQLite、Tantivy 及迁移遗留索引合计约 3.55 GB。该数据仅作后续体积优化基线，不代表 LogCrate 已达到 Everything 的空间效率。
- 2026-07-23 实际生成并检查 Windows debug 安装包：精简后的 NSIS 为 10,238,050 字节（约 9.76 MiB），仅包含一份 `logcrate_index_service.exe`，并包含 Orange GPLv3 与 UnRAR 许可证；MSI 约 16.2 MB 且包含索引服务。MSI/NSIS 已接入服务安装、修复与卸载钩子。发布前仍需使用正式证书验证服务签名和升级/卸载实机流程。
