...
```mermaid
flowchart LR
    A[下载目录 / 日志目录] -->|实时监控| B[发现 ZIP 与文本日志]
    B --> C[通知与目录定位]
    C --> D[免手动解压读取]
    D --> E[行索引 + 虚拟滚动]
    E --> F[多选项卡阅读]
...
- **ZIP 直接阅读**：像展开文件夹一样浏览 ZIP 内部条目，不产生散乱的手工解压目录。
...
> LogPeek 当前是**只读查看器**。可以重命名或删除磁盘文件，但不能编辑并保存日志正文，也不会改写 ZIP 内的内容。
...
LogPeek 基于系统 WebView 构建。Windows 10 / 11 通常已经包含 WebView2；缺失时系统会提示安装。
...
LogPeek 会保存监控配置，下次启动自动恢复。监控根会按父子关系去重：监控父目录后，不会重复监控它的子目录。
...
1. 在左侧目录树中点击裸日志文件；
2. 展开 ZIP，再点击其中的日志条目；
3. 从文件管理器拖入一个日志、ZIP 或文件夹。

拖入日志文件时，LogPeek 会自动添加其父目录、展开目录树、定位文件并打开正文。拖入 ZIP 或其它文件时会加入其所在目录的监控；拖入文件夹则监控该文件夹本身。
...
受监控目录出现新的 ZIP 或匹配后缀的日志后，右上角铃铛会显示未读数量。点击具体通知会逐层展开目录、定位目标；点击“全部已读”只清除通知，不会删除文件。
...
| 查看 ZIP 内日志 | 展开 ZIP 后点击条目 | 无需手工解压即可建立查看会话 |
...
| 完全退出 | 托盘菜单 → 退出 LogPeek | 停止监控并结束进程 |
...
- [ ] **选项卡工作区恢复**：重启后恢复上次打开的文件、活动选项卡和阅读位置。
- [ ] **书签与行标注**：收藏关键行、添加本地备注，并在文件变化后尽量恢复定位。
- [ ] **实时追踪追加内容**：为持续写入的裸日志提供类似 `tail -f` 的跟随模式。

### 长期：更多格式与更强工作流

- [ ] **更多归档格式**：tar.gz、7z，以及在授权和跨平台条件允许时支持 rar。
...
LogPeek 使用 Tauri 2 构建，前端负责交互与虚拟列表，Rust 后端负责文件系统监听、ZIP 读取、编码检测和行索引。
...
| ArchiveReader | ZIP 中央目录读取、归档条目流式访问与裸文本统一抽象 |
