...
> LogCrate 当前是**只读查看器**。可以重命名或删除磁盘文件，但不能编辑并保存日志正文，也不会改写 ZIP 内的内容。
...
拖入日志文件时，LogCrate 会自动添加其父目录、展开目录树、定位文件并打开正文。拖入 ZIP 或其它文件时会加入其所在目录的监控；拖入文件夹则监控该文件夹本身。
...
LogCrate 使用 Tauri 2 构建，前端负责交互与虚拟列表，Rust 后端负责文件系统监听、ZIP 读取、编码检测和行索引。
